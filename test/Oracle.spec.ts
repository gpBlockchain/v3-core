...
describe('Oracle', function () {
    ...
    const initializedOracleFixture = async () => {
        ...
        let tx = await oracle.initialize({
            time: 0,
            tick: 0,
            liquidity: 0,
        })
    ...
    describe('#initialize', () => {
        ...
        it('index is 0', async () => {
            let tx = await oracle.initialize({liquidity: 1, tick: 1, time: 1})
            await tx.wait()
        ...
        it('cardinality next is 1', async () => {
            let response = await oracle.initialize({liquidity: 1, tick: 1, time: 1})
            await response.wait()
        ...
        it.skip('gas(local eth run failed : gas not same)', async () => {
    ...
    describe('#grow', () => {
        ...
        it('grow after wrap', async () => {
            let tx = await oracle.grow(2)
            await tx.wait()
            tx = await oracle.update({advanceTimeBy: 2, liquidity: 1, tick: 1}) // index is now 1
            await tx.wait()
            tx = await oracle.update({advanceTimeBy: 2, liquidity: 1, tick: 1}) // index is now 0 again
            await tx.wait()
            expect(await oracle.index()).to.eq(0)
            tx = await oracle.grow(3)
            await tx.wait()
            expect(await oracle.index()).to.eq(0)
        ...
        it.skip('gas for growing by 1 slot when index == cardinality - 1(gas not eq)', async () => {
            await snapshotGasCost(oracle.grow(2))
        })

        it.skip('gas for growing by 10 slots when index == cardinality - 1', async () => {
            await snapshotGasCost(oracle.grow(11))
        })

        it.skip('gas for growing by 1 slot when index != cardinality - 1', async () => {
            let tx = await oracle.grow(2)
            await tx.wait()
    ...
    describe('#write', () => {
        ...
        it('single element array gets overwritten', async () => {
            let tx = await oracle.update({advanceTimeBy: 1, tick: 2, liquidity: 5})
            await tx.wait()
            expect(await oracle.index()).to.eq(0)
            checkObservationEquals(await oracle.observations(0), {
                initialized: true,
            ...
            tx = await oracle.update({advanceTimeBy: 5, tick: -1, liquidity: 8})
            await tx.wait()
            expect(await oracle.index()).to.eq(0)
            checkObservationEquals(await oracle.observations(0), {
                initialized: true,
            ...
            tx = await oracle.update({advanceTimeBy: 3, tick: 2, liquidity: 3})
            await tx.wait()
            expect(await oracle.index()).to.eq(0)
            checkObservationEquals(await oracle.observations(0), {
                initialized: true,
        ...
        it('does nothing if time has not changed', async () => {
            let tx = await oracle.grow(2)
            await tx.wait()
            tx = await oracle.update({advanceTimeBy: 1, tick: 3, liquidity: 2})
            await tx.wait()
            expect(await oracle.index()).to.eq(1)
            tx = await oracle.update({advanceTimeBy: 0, tick: -5, liquidity: 9})
            await tx.wait()
            expect(await oracle.index()).to.eq(1)
        })
        ...
        it('writes an index if time has changed', async () => {
            let tx = await oracle.grow(3)
            await tx.wait()
            tx = await oracle.update({advanceTimeBy: 6, tick: 3, liquidity: 2})
            await tx.wait()
            expect(await oracle.index()).to.eq(1)
            tx = await oracle.update({advanceTimeBy: 4, tick: -5, liquidity: 9})
            await tx.wait()
        ...
        it('grows cardinality when writing past', async () => {
            let tx = await oracle.grow(2)
            await tx.wait()
            tx = await oracle.grow(4)
            await tx.wait()
            expect(await oracle.cardinality()).to.eq(1)
            tx = await oracle.update({advanceTimeBy: 3, tick: 5, liquidity: 6})
            await tx.wait()
            expect(await oracle.cardinality()).to.eq(4)
            tx = await oracle.update({advanceTimeBy: 4, tick: 6, liquidity: 4})
            await tx.wait()
            expect(await oracle.cardinality()).to.eq(4)
            expect(await oracle.index()).to.eq(2)
        ...
        it('wraps around', async () => {
            let tx = await oracle.grow(3)
            await tx.wait()
            tx = await oracle.update({advanceTimeBy: 3, tick: 1, liquidity: 2})
            await tx.wait()
            tx = await oracle.update({advanceTimeBy: 4, tick: 2, liquidity: 3})
            await tx.wait()
            tx = await oracle.update({advanceTimeBy: 5, tick: 3, liquidity: 4})
            await tx.wait()
        ...
        it('accumulates liquidity', async () => {
            let tx = await oracle.grow(4)
            await tx.wait()
            tx = await oracle.update({advanceTimeBy: 3, tick: 3, liquidity: 2})
            await tx.wait()
            tx = await oracle.update({advanceTimeBy: 4, tick: -7, liquidity: 6})
            tx = await oracle.update({advanceTimeBy: 5, tick: -2, liquidity: 4})
            await tx.wait()
    ...
    describe('#observe', () => {
        describe('before initialization', async () => {
            ...
            it('fails if an older observation does not exist', async () => {
                let tx = await oracle.initialize({liquidity: 4, tick: 2, time: 5})
                await tx.wait()
                await expect(observeSingle(1)).to.be.revertedWith('OLD')
            })

            it('does not fail across overflow boundary', async () => {
                let tx = await oracle.initialize({liquidity: 4, tick: 2, time: 2 ** 32 - 1})
                await tx.wait()
            ...
            it.skip('gas for observe since most recent', async () => {
                await (await oracle.initialize({liquidity: 5, tick: -5, time: 5})).wait()
                await (await oracle.advanceTime(2)).wait()
                await snapshotGasCost(oracle.getGasCostOfObserve([1]))
            })

            it.skip('gas for single observation at current time', async () => {
                await oracle.initialize({liquidity: 5, tick: -5, time: 5})
                await snapshotGasCost(oracle.getGasCostOfObserve([0]))
            })

            it.skip('gas for single observation at current time counterfactually computed', async () => {
                await oracle.initialize({liquidity: 5, tick: -5, time: 5})
        ...
        describe('query', function () {
            for (const startingTime of [5, 2 ** 32 - 5]) {
                describe(`initialized with 5 observations with starting time of ${startingTime}`, () => {
                    ...
                    let oracle: OracleTest
                    before('set up observations', async () => {
        ...
        describe('query advance 5', function () {
            for (const startingTime of [5, 2 ** 32 - 5]) {
                describe(`initialized with 5 observations with starting time of ${startingTime}`, () => {
                    const oracleFixture5Observations = async () => {
                        const oracle = await oracleFixture()
                    ...
                    describe('advanceTime 5', function () {
        ...
        describe('query advance 6', function () {
            for (const startingTime of [5, 2 ** 32 - 5]) {
                describe(`initialized with 5 observations with starting time of ${startingTime}`, () => {
                    const oracleFixture5Observations = async () => {
                        const oracle = await oracleFixture()
                    ...
                    describe('advanceTime 6', function () {
                        before(async () => {
